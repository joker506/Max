    ...
    <section class="sign-up">
      <div class="sign-up__wrap">
        ...
        <div class="btn__wrap">
          <Button
            class="sign-up__btn"
            @click="changeStep('MerchantStep2')"
...
export default {
  ...
  methods: {
    ...
    changeStep(step) {
      if (this.$v.$invalid) {
        ...
        this.$store.commit("signUp/SIGN_UP_MERCHANT", {
          identifier: this.phone,
          profile_group: this.profileGroup,
          profile_type: this.profileType,
        });
        this.$bus.emit("CHANGE_SIGN_UP_STEP", step);
        //поправить переход при успешном статусе
        this.$store
  ...
  computed: {
    ...mapState({
      ...
      profileGroup: (state) => state.signUp.role.profile_group,
    }),
    ...mapGetters({
      profileType: "signUp/typeRole",
    }),
  },
  mounted() {
